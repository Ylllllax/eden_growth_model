...
class DataAnalysis:
    ...
    def log_log_origin(self, initial_power, data_points, simulations, L_min):
        
        other=BallisticDeposition(L_min)
        tracker=IterationTracker(number_of_system_sizes=5, total_data_points=data_points, total_simulations=simulations)        
        m=0
        while m<5:
            #print m,m,m,m,m,m,m,m,m,m,m,m,m,m,m,m,m,m,m,m,m,m,m,m
            
            x, t= self.roughness_dynamics_average(other, tracker, initial_power, data_points, simulations)
            log_t=np.log(t)
            log_w=np.log(x)
