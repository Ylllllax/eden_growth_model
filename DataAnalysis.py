...
class DataAnalysis:
    ...
    def roughness_dynamics(self, other, tracker, initial_power,
                           data_points):  # generates a series of roughness values for a series of matrices
        ...
        while m <= data_points + initial_power:
            ...
            x = np.append(x,
                          other.roughness())  # appending the roughnesses of system after each set of 2**m particles have been deposited
    ...
    def log_log_origin_average(self, other, initial_power, data_points, simulations, system_sizes):
        start = np.datetime64(datetime.datetime.now())
        # other=BallisticDeposition(L_min)
        # other=EdenCsystem(L_min)
        tracker = IterationTracker(number_of_system_sizes=system_sizes, total_data_points=data_points,
                                   total_simulations=simulations)
        m = 0
        while m < system_sizes:
            x, t, upper_error, lower_error = self.roughness_dynamics_average(other, tracker, initial_power, data_points,
                                                                             simulations)
            log_t = np.log(t)
            log_w = np.log(x)
    ...
    def log_log_origin_partial(self, other, initial_power, data_points, simulations, system_sizes):
        start = np.datetime64(datetime.datetime.now())
        tracker = IterationTracker(number_of_system_sizes=system_sizes, total_data_points=data_points,
                                   total_simulations=simulations)
        m = 0
        while m < system_sizes:
            ...
            np.savetxt("(%s, %g, %g, %g, %g, %s).txt" % (other.__class__.__name__, other.system_size(), initial_power, data_points, simulations, d), np.transpose(text_list))
            m = m + 1
            other.reset_system(other.system_size() + 500)  # increasing system by 500 each time
        end = np.datetime64(datetime.datetime.now())
    ...
    def log_log_line_plot(self, other, x_array, y_array, line_of_best_fit=None):
        ...
        if line_of_best_fit == True:
            ...
            ax.text(0.1, 0.9, r'$\beta=$%g' % (m), style='italic', horizontalalignment='left', verticalalignment='top',
                    transform=ax.transAxes)
        ax.text(0.1, 0.85, 'Particles Deposited=%g' % (other.number_of_deposited_particles()), style='italic',
                horizontalalignment='left', verticalalignment='top',
                transform=ax.transAxes)  # position of test and the test itself
        ax.text(0.1, 0.8, 'System Size=%g' % (other.system_size()), style='italic', horizontalalignment='left',
...
class IterationTracker:
    ...
    def display(self, other):
        print "%s %g, %s %g %s %g, %s %g %s %g" % (
        "system size =", other.system_size(), "simulation number", self.__simulation_number, "out of",
        self.__total_simulations, "data point", self.__data_point_number, "out of", self.__total_data_points)
